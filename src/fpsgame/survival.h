...
{
    { WEAP_BITE,        18, 150, 6, 80,  100, 800, 1, 12, 100, S_PAINO, S_DIE1,   "zombie bike",        "playermodels/zombies/nazizombiebike",  NULL},
    { WEAP_PISTOL,      18, 150, 6, 100, 300, 400, 4, 13, 115, S_PAINE, S_DEATHE, "nazi zombie",        "playermodels/zombies/nazizombie",      "vwep/pistol"},
    { WEAP_BITE,        18, 150, 6, 100, 300, 400, 4, 12, 115, S_PAINE, S_DEATHE, "nazi zombie 2",      "playermodels/zombies/nazizombie2",     NULL},
    { WEAP_BITE,        20, 120, 6, 0,   100, 400, 1, 11,  90, S_PAINP, S_PIGGR2, "fast zombie",        "playermodels/zombies/fastzombie",      NULL},
    { WEAP_BITE,        13, 120, 6, 0,   100, 400, 1, 12,  50, S_PAINS, S_DEATHS, "female",             "playermodels/zombies/female",          NULL},
    { WEAP_BITE,        13, 120, 6, 0,   100, 400, 1,  9,  50, S_PAINS, S_DEATHS, "female 2",           "playermodels/zombies/female2",         NULL},
    ...
    { WEAP_ROCKETL,     13, 450, 1, 0,   100, 400, 1, 24, 200, S_PAIND, S_DEATHD, "zombie boss",        "playermodels/zombies/zombieboss",      "vwep/rocket"},
...
static int bestenemy;
...
#endif
    {
        ...
        vec stackpos;
        int enemy;                      // zombie wants to kill this entity
        int stacked;
#endif
        ...
        void zombieaction(int curtime)           // main AI thinking routine, called every frame for every monster
        {
            ...
            float dist = enemyP->o.dist(o);
            if(zombiestate!=M_SLEEP) pitch = asin((enemyP->o.z - o.z) / dist) / RAD; 
            ...
            float enemyyaw = -atan2(enemyP->o.x - o.x, enemyP->o.y - o.y)/RAD;
            ...
            switch(zombiestate)
            {
                ...
                case M_SLEEP:                       // state classic sp monster start in, wait for visual contact
                {
                    ...
                    if(
                        dist<32                   // the better the angle to the player, the further the monster can see/hear
                        ||(dist<64 && angle<135)
                        ||(dist<128 && angle<90)
                        ||(dist<256 && angle<45)
                        || angle<10
                    ...
                    {
                        vec target;
                        if(raycubelos(o, enemyP->o, target))
                        {
                ...
                case M_HOME:                        // monster has visual contact, heads straight for player and may want to shoot at any time
                    ...
                    {
                        vec target;
                        if(!raycubelos(o, enemyP->o, target))    // no visual contact anymore, let monster get as close as possible then search for player
                        {
            ...
            if(move || maymove() || (
                stacked >= 0 && getclient(stacked) != NULL &&
                (
                    getclient(stacked)->state != CS_ALIVE
                    || stackpos != getclient(stacked)->o
                )
                ))
            {
...
int newround(bool force = false)
{
    ...
    static void stackzombie(zombie *d, physent *o)
    {
        ...
        loopv(zombies)
        {
            if((physent *)zombies[i] == o)
            {
                cn = zombies[i]->clientnum;
            }
        }
        if(cn == -1) loopv(players)
        {
            if((physent *)players[i] == o)
            {
                cn = zombies[i]->clientnum;
            }
        }
...
//}
    ...
    {
        void *vec = (void *)(&zombies);
        return *(vector<dynent *>*)vec;
    }
    ...
    void message(int type, ucharbuf &p)
    {
        ...
        switch(type)
        {
            ...
            {
                int cn;
                fpsent *zi = getclient(cn = getint(p));
                if(zi != NULL)
                {
                    zi->ownernum = getint(p);
                }
                ...
                {
                    printf("Server tries to assign invalid zombie: %i\n", cn);
                }
                break;
            }
            ...
            {
                int cn;
                zi = (zombie*)getclient(cn = getint(p));
                if(zi != NULL)
                {
                    zi->spawn(getint(p), vec(0, 0, 0));
                    zi->ownernum = getint(p);
                ...
                {
                    printf("Server tries to spawn invalid zombie: %i\n", cn);
                }
                break;
            }
            case N_SURVNEWROUND:
...
{
    survivalclientmode::stackzombie((zombie *)d, o);
}
#endif
