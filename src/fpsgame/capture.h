...
#endif
{
    ...
    void rendertether(fpsent *d)
    {
        ...
        if(d->state==CS_ALIVE)
        {
            loopv(bases)
            {
                ...
                particle_flare(pos, vec(b.ammopos.x, b.ammopos.y, b.ammopos.z - AMMOHEIGHT - 4.4f), 0, PART_LIGHTNING, strcmp(d->team, TEAM_0) ? 0xFF2222 : 0x2222FF, 1.0f);
                if(oldbase < 0)
                {
                    particle_fireball(pos, 4.8f, PART_EXPLOSION, 250, strcmp(d->team, TEAM_0) ? 0x802020 : 0x2020FF, 4.8f);
                    particle_splash(PART_SPARK, 50, 250, pos, strcmp(d->team, TEAM_0) ? 0x802020 : 0x2020FF, 0.24f);
                }
                d->lastbase = i;
            }
        }
    ...
    void rendergame()
    {
        ...
        //    }
            ...
            if(b.owner[0])
            {
                bool isowner = !strcmp(b.owner, TEAM_0);
            ...
            else if(b.enemy[0])
            {
                ...
                if(strcmp(b.enemy, TEAM_0)) { tcolor = 0xFF4B19; mtype = PART_METER; mcolor = 0xFF1932; }
    ...
    void drawRadarBlips(fpsent *d, float blipsize, int fw, int fh, int type, bool skipenemy = false)
    {
    ...
    void drawhud(fpsent *d, int w, int h)
    {
        ...
        int fw = 1, fh = 1;
        bool isTeam0 = strcmp(TEAM_0, player1->team) == 0;
        if(basenumbers)
        {
            pushfont();
            setfont(isTeam0 ? "digit_blue": "digit_red");
            text_bounds(" ", fw, fh);
        }
        else settexture(isTeam0 ? "data/hud/blip_blue.png" : "data/hud/blip_red.png", 3);
        glPushMatrix();
        glTranslatef(x + 0.5f*s, y + 0.5f*s, 0);
        float blipsize = basenumbers ? 0.1f : 0.05f;
        glScalef((s*blipsize)/fw, (s*blipsize)/fh, 1.0f);
        drawRadarBlips(d, blipsize, fw, fh, 1, showenemies);
        if(basenumbers) setfont("digit_grey");
        else settexture("data/hud/blip_grey.png", 3);
        drawRadarBlips(d, blipsize, fw, fh, 0, showenemies);
        if(basenumbers) setfont(isTeam0 ? "digit_red" : "digit_blue");
        else settexture(isTeam0 ? "data/hud/blip_red.png" : "data/hud/blip_blue.png", 3);
        drawRadarBlips(d, blipsize, fw, fh, -1, showenemies);
        if(showenemies) drawRadarBlips(d, blipsize, fw, fh, -2);
        glPopMatrix();
    ...
    void updatebase(int i, const char *owner, const char *enemy, int converted, int ammo)
    {
        ...
        if(strcmp(b.owner, owner)) particle_splash(PART_SPARK, 200, 250, b.ammopos, owner[0] ? (strcmp(owner, TEAM_0) ? 0x802020 : 0x2020FF) : 0x208020, 0.24f);
        copystring(b.owner, owner);
        copystring(b.enemy, enemy);
        b.converted = converted;
        // todo-shell
    ...
    void setscore(int base, const char *team, int total)
    {
        ...
        else if(bases.inrange(base))
        {
            ...
            if(!strcmp(b.owner, team))
            {
                ...
                particle_textcopy(above, msg, PART_TEXT, 2000, isteam(team, TEAM_0) ? 0x6496FF : 0xFF4B19, 4.0f, -8);
            }
        }
    }
    ...
    void initclient(clientinfo *ci, packetbuf &p, bool connecting)
    {
        ...
        {
            baseinfo &b = bases[i];
            putint(p, min(max(b.ammotype, 1), I_HEALTH+1)); // todo-shell
    ...
    {
        if(notgotbases || ci->gameclip) return false;
        leavebases(ci->team, ci->state.o);
        return false;
    }
    ...
    void parsebases(ucharbuf &p, bool commit)
    {
        ...
        loopi(numbases)
        {
            ...
            if(p.overread()) break;
            if(commit && notgotbases) addbase(ammotype>=WEAP_SLUGSHOT && ammotype<=WEAP_PISTOL ? ammotype : min(ammotype, 0), o);
        }
...
case N_BASES:
    if(smode==&capturemode) capturemode.parsebases(p, (ci->state.state!=CS_SPECTATOR || ci->privilege >= PRIV_MASTER || ci->local) && !strcmp(ci->clientmap, smapname));
    break;

case N_REPAMMO:
    if((ci->state.state!=CS_SPECTATOR || ci->local || ci->privilege  >= PRIV_MASTER) && cq && smode==&capturemode) capturemode.replenishammo(cq);
    break;
...
case N_BASEREGEN:
{
    ...
    if(regen && m_capture)
    {
        regen->health = health;
        regen->armour = armour;
        if(ammotype>=WEAP_SLUGSHOT && ammotype<=WEAP_PISTOL) regen->ammo[ammotype] = ammo;
    }
    break;
}
