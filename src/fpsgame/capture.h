...
//            particle_fireball(b.ammopos, 4.8f, PART_EXPLOSION, 0, b.owner[0] ? (strcmp(b.owner, player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 4.8f);
			...
			loopj(b.ammo)
            {
                ...
                ammopos.z += 4;
                rendermodel(entities::entmdlname(AMMO_L1+b.ammotype-1), ANIM_MAPMODEL|ANIM_LOOP, ammopos, 0, 0, MDL_CULL_VFC | MDL_CULL_OCCLUDED);
            }
        ...
        loopv(clients)
        {
            ...
            if(ci->state.state==CS_ALIVE && ci->team[0] && !strcmp(ci->team, b.owner) && insidebase(b, ci->state.o))
            {
                ...
                if(ci->state.armourtype == 0)
                {
                    ...
                    //ci->state.armour = min(ci->state.armour + ticks*REGENARMOUR, itemstats[I_GREENARMOUR-AMMO_L1].max);
                    notify = true;
                }
                if(b.valid())
                {
