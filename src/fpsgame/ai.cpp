...
namespace ai
{
	...
	bool respawnwait(fpsent *d)
	{
		if(!d)return false;
		if(d->lastpain == 0){return true;} //d->lastpain = lastmillis-(10*1000);
		return max(0, 10-(lastmillis-d->lastpain)/1000) <= 0 ? true : false;
	}
    void think(fpsent *d, bool run)
    {
        ...
        loopvrev(d->ai->state)
        {
            ...
            if(d->state == CS_DEAD && d->respawned!=d->lifesequence && ((!cmode && respawnwait(d)) || (cmode && cmode->respawnwait(d) <= 0)) && lastmillis - d->lastpain >= 500)
            {
