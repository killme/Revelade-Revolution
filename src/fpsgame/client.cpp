...
namespace game
{
    ...
    {
        sendcrc = true;
        if(player1->state!=CS_SPECTATOR || player1->privilege >= PRIV_MASTER || !remote) senditemstoserver = true;
    }
    ...
    struct mapvote
    {
        ...
        static int compare(const mapvote *a, const mapvote *b)
        {
            if(sortvotes)
            {
