...
namespace entities
{
    ...
    {
        int t = ents[i]->type;
        if(t<I_AMMO || t>I_QUAD) return NULL;
    ...
    const char *entmdlname(int type)
    {
        static const char * const entmdlnames[] =
        {
            ...
            "gamemodels/health", "gamemodels/health", "gamemodels/health", "gamemodels/ammo/mortar",
            "gamemodels/armor/green", "gamemodels/armor/yellow", "gamemodels/quad", "teleporter",
            NULL, NULL,
            "carrot",
            NULL, NULL,
            "checkpoint",
        ...
        loopv(ents)
        {
            ...
            switch(e.type)
            {
                ...
                case FLAG:
                    if(editmode)
                    {
                        rendermodel(&e.light, (e.attr2 == 1 ? RR_MODEL_FLAG_BLUE : (e.attr2 == 2 ? RR_MODEL_FLAG_RED : RR_MODEL_FLAG_NEUTRAL)), ANIM_MAPMODEL|ANIM_LOOP,
                                    e.o, e.attr1, 0,
                                    MDL_DYNSHADOW | MDL_CULL_VFC | MDL_CULL_OCCLUDED,
                                    NULL, NULL, 0, 0, 0.3f);
                    }
                    break;
                default:
            ...
            {
                vec p = e.o;
                p.z += 1+sinf(lastmillis/200.0+e.o.x+e.o.y)/1;
    ...
    void trypickup(int n, fpsent *d)
    {
        switch(ents[n]->type)
        {
            ...
            case JUMPPAD:
            {
                ...
                if(d->ai.local && d->ai.type == ai::AI_TYPE_BOT) ai::bot::getBotState(d)->becareful = true;
                d->falling = vec(0, 0, 0);
