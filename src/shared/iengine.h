...
extern char *bsvariable(const char *name, const char *cur, char **storage, void (*fun)(), int flags);
...
extern void newgui(char *name, char *contents, char *initscript = NULL, char *header = NULL);
...
extern void newentity(int type, int a1, int a2, int a3, int a4, int a5, bool custom = false, vec o = vec(0, 0, 0));
...
extern int draw_text(const char *str, int left, int top, int r = 255, int g = 255, int b = 255, int a = 255, int flags = TEXT_SHADOW, int cursor = -1, int maxwidth = -1);
...
extern int text_visible(const char *str, int hitx, int hity, int maxwidth, int flags = 0);
...
extern void particle_trail(int type, int fade, const vec &from, const vec &to, int color = 0xFFFFFF, float size = 1.0f, int gravity = 20, bool bubbles = false, int radius = 30);
...
struct g3d_gui
{
    ...
    virtual void listbox(const char *items, int color, int width = 0, int height = 0) = 0;
    ...
    virtual void vslider(int &val, int vmin, int vmax, int color, char *label = NULL) = 0;
    ...
    virtual int playerpreview(float scale, bool overlaid = false) { return 0; }
