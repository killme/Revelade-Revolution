...
extern void settextscale(float scale);
...
extern int draw_text(const char *str, int left, int top, int r, int g, int b, int a, int flags, int cursor, int maxwidth); //RR
extern int draw_textx(const char *fstr, int left, int top, int r = 255, int g = 255, int b = 255, int a = 255, int flags = 0, int cursor = -1, int maxwidth = -1, ...) PRINTFARGS(1, 11); //RR
...
// RR -> SE particles (TODO: reimplement)
inline void regular_particle_splash(int type, int num, int fade, const vec &p, int color, float size, int radius, int gravity, int delay, bool hover, int grow = 0)
{
    regular_particle_splash(type, num, fade, p, color, size, radius, gravity, delay);
}
inline void particle_splash(int type, int num, int fade, const vec &p, int color, float size, int radius, int gravity, bool regfade, int flag = 0, bool fastsplash = false, int grow = 0)
{
    particle_splash(type, num, fade, p, color, size, radius, gravity);
}
inline void particle_trail(int type, int fade, const vec &from, const vec &to, int color, float size, int gravity, bool bubbles, int radius = 30)
{
...
// RR -> SE particles (TODO: reimplement)
inline void particle_flare(const vec &p, const vec &dest, int fade, int type, int color, float size, physent *owner, int grow)
{
    particle_flare(p, dest, fade, type, color, size, owner);
}
// </RR>
...
extern void renderclient(dynent *d, const char *mdlname, modelattach *attachments, int hold, int attack, int attackdelay, int lastaction, int lastpain, float fade = 1, bool ragdoll = false, bool py = false);
...
struct g3d_gui
{
    ...
    virtual int text(const char *text, int color, const char *icon = NULL, int minwidth = 0) = 0;
    ...
    virtual int button(const char *text, int color, const char *icon = NULL, int minwidth = 0) = 0;
    int buttonf(const char *fmt, int color, const char *icon = NULL, ...) PRINTFARGS(2, 5)
    {
        defvformatstring(str, icon, fmt);
        return button(str, color, icon);
    }
    virtual int title(const char *text, int color, const char *icon = NULL, int minwidth = 0) = 0;
    ...
    virtual int image(Texture *t, float scale, bool overlaid = false, int colour = 0xFFFFFF) = 0;
