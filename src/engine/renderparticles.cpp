...
struct varenderer : partrenderer
{
    ...
    void genverts(particle *p, partvert *vs, bool regen)
    {
        ...
        if(regen)
        {
            ...
            {
                float tx = 0.125f*(p->flags&3), ty = 0.125f*((p->flags>>2)&3);
                SETTEXCOORDS(tx, tx + 0.125f, ty, ty + 0.125f, {});
            }
...
static partrenderer *parts[] =
{
    ...
    new quadrenderer("<grey>data/particles/wisp.png", PT_PART|PT_FLIP|PT_RND4|PT_ROT|PT_EXPAND),  // smoke
    new quadrenderer("<grey>data/particles/flames.png", PT_PART|PT_FLIP|PT_RND4|PT_GLARE),          // flame on - no flipping please, they have orientation
