...
extern const float GRAVITY = 175.0f;
...
VARP(quakefade, 0, 500, 1000);
FVARP(quakemax, 0, 6.f, 100.f);
...
bool moveplayer(physent *pl, int moveres, bool local, int curtime)
{
    ...
    if (pl->quake)
    {
        int qu = (int)ceil(min(pl->quake/(float)quakefade, 1.0f) *quakemax);
        pl->roll += rnd(qu)-(qu/2.f);
        pl->quake = max(pl->quake-curtime, 0);
    }
...
bool reflect(physent *d, float elasticity, float waterfric)
{
    //if(physsteps <= 0)
    //{
    //    interppos(d);
    ...
    //float secs = physframetime/1000.0f;
...
ICOMMAND(attack, "Di", (int *down, int *altfire), { game::doattack(*down!=0, *altfire!=0); });
...
bool entinmap(dynent *d, bool avoidplayers)        // brute force but effective way to find a free spawn spot in the map
{
    ...
    d->o = orig;
    d->resetinterp();
    conoutf(CON_WARN, "can't find entity spawn spot! (%.1f, %.1f, %.1f) (%d, %d, %d, %s)", d->o.x, d->o.y, d->o.z, d->type, avoidplayers, d->state, game::getentname(d)); //@todo: remove debugging vars
    return false;
}
