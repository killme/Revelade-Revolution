...
static void setalias(const char *name, tagval &v)
{
    ...
    {
        addident(ident(ID_ALIAS, newstring(name), v, identflags & ~IDF_PERSIST)); // RR -> do not persist anything automatically
    }
}
...
void persist(const char *name, bool value)
{
    ident *i = idents.access(name);
    if(i && i->type == ID_ALIAS) i->flags = (i->flags & ~IDF_PERSIST) | (value ? IDF_PERSIST : 0);
}
...
ICOMMAND(error, "C", (char *s), conoutf(CON_ERROR, "\fr%s", s));
...
ICOMMAND(toupper, "s", (char *s), char *commandret = newstring(s); for(;*commandret;commandret++) *commandret = toupper(*commandret); commandret -= strlen(s); stringret(commandret));
ICOMMAND(tolower, "s", (char *s), char *commandret = newstring(s); for(;*commandret;commandret++) *commandret = tolower(*commandret); commandret -= strlen(s); stringret(commandret));
