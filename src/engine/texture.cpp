...
static const char *colirizables[] = {
    "action.png", "arrow_bw.png", "arrow_fw.png", "buy.png", "checkbox_off.png", "checkbox_on.png", "checkbox_dis.png",
    "cube.png", "exit.png", "font.png", "info.png", "menu.png", "radio_off.png", "radio_on.png", "radio_dis.png", "spectator.png"
};
static int colirizableslen = sizeof(colirizables)/sizeof(colirizables[0]);

inline bool iscolorizable(const char *name)
{
    loopi(colirizableslen) if (!strcmp(name, colirizables[i])) return true;
    return false;
}
...
static Texture *newtexture(Texture *t, const char *rname, ImageData &s, int clamp = 0, bool mipit = true, bool canreduce = false, bool transient = false, int compress = 0)
{
    ...
    if (rname)
    {
        char *path = (char *)rname, *pathend = path;
        do { if (*path == '/' || *path == '\\') pathend = path; } while (*++path != '\0');
        if (*pathend) pathend++;

        t->colorize = iscolorizable(pathend);
    }
...
void takemapshot()
{
    static string buf;
    int format = -1;
    formatstring(buf)("packages/base/%s", game::getclientmap());
    format = IMG_PNG;
    concatstring(buf, imageexts[format]);
    ...
    conoutf("Mapshot saved to: %s", buf);
}
