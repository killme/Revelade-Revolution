...
void settextscale(float s) { gtextscale = s; }
///RR

void newfont(char *name, char *tex, int *canbedefault, int *defaultw, int *defaulth)
{
    font *f = &fonts[name];
...
static vector<SavedFont> fontstack;

void pushfont()
{
    SavedFont &f = fontstack.add();
    f.f = curfont;
    f.scale = gtextscale;
}
...
static void text_color(char c, char *stack, int size, int &sp, bvec color, int a) 
{
    ...
    else
    {
        ...
        switch(c)
        {
            ...
            case 'e': case 'E': a -= c!='E' ? a/2 : a/4; break;
            case '-': break; // Default color // RR -> made explicit
...
#define TEXTSKELETON \
    float y = 0, x = 0, scale = (curfont->scale*gtextscale)/float(curfont->defaulth);\
    int i;\
    for(i = 0; str[i]; i++)\
    {\
        TEXTINDEX(i)\
...
void draw_text(const char *str, int left, int top, int r, int g, int b, int a, int cursor, int maxwidth) 
{
    ...
    #define TEXTWORD TEXTWORDSKELETON
    char colorstack[10];
    colorstack[0] = '-'; //indicate user color // RR -> 'c' -> '-'
    bvec color(r, g, b);
    int colorpos = 0;
    float cx = -FONTW, cy = 0;
    bool usecolor = true;
...
{
    draw_text(str, left, top, r, g, b, a, cursor, maxwidth);
    return FONTH;
}
