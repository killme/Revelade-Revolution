    ...
    struct md5meshgroup : skelmeshgroup
    {
        ...
        skelanimspec *loadmd5anim(const char *filename)
        {
            ...
            while(f->getline(buf, sizeof(buf)))
            {
                ...
                if(sscanf(buf, " MD5Version %d", &tmp)==1)
                {
                    if(tmp!=10) { conoutf("invalid version\n"); delete f; return NULL; }
                }
                else if(sscanf(buf, " numJoints %d", &tmp)==1)
                {
                    if(tmp!=skel->numbones) { conoutf("invalid join count (%i!=%i)\n", tmp, skel->numbones); delete f; return NULL; }
                }
                else if(sscanf(buf, " numFrames %d", &animframes)==1)
                {
                    if(animframes<1) { conoutf("invalid frames count\n"); delete f; return NULL; }
                }
                ...
                else if(strstr(buf, "baseframe {"))
                {
                    ...
                    if(basejoints.length()!=skel->numbones) { conoutf("joints != bones\n"); delete f; return NULL; }
