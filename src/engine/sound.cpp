...
void startmusic(char *name, char *cmd)
{
    ...
    if(soundvol && musicvol && *name)
    {
        ...
        {
            conoutf(CON_ERROR, "\frcould not play music: \fo%s -> %s", file, Mix_GetError());
            intret(0); 
        }
    }
}
...
void drawsounds()
{
    ...
    {
        soundchannel &chan = channels[i];
        if(chan.slot && chan.hasloc())
        {
            int radius = maxsoundradius;
            if(chan.radius > 0) radius = maxsoundradius ? min(maxsoundradius, chan.radius) : chan.radius;
            radius = clamp(radius, 2, 10);
            vec bottom(chan.loc), top(chan.loc);
            top.z += radius;
            bottom.z -= radius;

            vec direction(0.f, 0.f);
            direction.z = 0;
            ...
            loopi(8)
            {
                ...
                vec point(direction);
                point.rotate_around_z(angle);
                point.add(chan.loc);

                particle_flare(top, point, 0, PART_DEBUG_LINE, 0xFFAAAA, 0.50f);
                particle_flare(bottom, point, 0, PART_DEBUG_LINE, 0xFFAAAA, 0.50f);
            }
        }
    }
}
...
int playsound(int n, const vec *loc, extentity *ent, int flags, int loops, int fade, int chanid, int radius, int expire)
{
    ...
    if(!sounds.inrange(n)) { conoutf(CON_WARN, "unregistered sound (%i): %d (/%d)", flags&SND_MAP, n, sounds.length()); return -1; }
    soundconfig &config = sounds[n];
...
void initmumble()
{
    ...
    if(VALID_MUMBLELINK) return;
        ...
        if(mumblelink)
        {
            ...
            if(mumbleinfo) wcsncpy(mumbleinfo->name, L"Revelade Revolution", 256);
        }
    #elif defined(_POSIX_SHARED_MEMORY_OBJECTS)
        ...
        if(mumblelink >= 0)
        {
            ...
            if(mumbleinfo != (MumbleInfo *)-1) wcsncpy(mumbleinfo->name, L"Revelade Revolution", 256);
        }
    #endif
