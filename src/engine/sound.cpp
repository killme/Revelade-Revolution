...
int playsound(int n, const vec *loc, extentity *ent, int loops, int fade, int chanid, int radius, int expire)
{
    ...
    if(!slot.sample->chunk)
    {
        ...
        bool inData = 0 == strncmp(slot.sample->name, dataPath, strlen(dataPath));
        
        const char *exts[] = { "", ".wav", ".ogg" };
        string buf;
        loopi(sizeof(exts)/sizeof(exts[0]))
        {
            formatstring(buf)(inData ? "%s%s" : "data/sounds/%s%s", slot.sample->name, exts[i]);
            path(buf);
            slot.sample->chunk = loadwav(buf);
            if(slot.sample->chunk) break;
        }

        if(!slot.sample->chunk) { conoutf(CON_ERROR, "\frfailed to load sample: \fo%s", buf); return -1; }
    }
