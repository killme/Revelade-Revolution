...
#include "textedit.h"
#include "translate.h" //RR -> translatable guis
...
struct gui : g3d_gui
{
    ...
    void progress(float percent) { autotab(); line_((FONTH*2)/5, percent); } // RR -> Less high progress bars (?)
    ...
    static void drawskin(int x, int y, int gapw, int gaph, int start, int n, int passes = 1, const vec &light = vec(1, 1, 1), float alpha = 0.80f)//int vleft, int vright, int vtop, int vbottom, int start, int n)
    {
        ...
        loopj(passes)
        {
            ...
            glColor4f(j ? light.x : guimenucolorr/255.0f, j ? light.y : guimenucolorg/255.0f, j ? light.z : guimenucolorb/255.0f, passes<=1 || j ? alpha : alpha/2); //ghost when its behind something in depth // RR -> apply menu color
            loopi(n)
            {
    ...
    void start(int starttime, float initscale, int *tab, bool allowinput)
    {
        ...
        else
        {
            ...
            drawwatermark(curx-skinx[2]*SKIN_SCALE, cury-skiny[6]*SKIN_SCALE, xsize, ysize); // RR -> Draw watermark
        }
    }
