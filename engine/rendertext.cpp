...
static void text_color(char c, char *stack, int size, int &sp, bvec color, int a) 
{
    ...
    else
    {
        ...
        switch(c)
        {
            ...
            case '1': case 'b': color = bvec( 96, 160, 255); break;   // blue: "echo" command
            case '2': case 'y': color = bvec(255, 192,  64); break;   // yellow: gameplay messages 
            case '3': case 'e': color = bvec(255,  64,  64); break;   // red: important errors
            case '4': case 'a': color = bvec(128, 128, 128); break;   // gray
            case '5': case 'm': color = bvec(192,  64, 192); break;   // magenta
            case '6': case 'o': color = bvec(255, 128,   0); break;   // orange
            case '7': case 'w': color = bvec(255, 255, 255); break;   // white
    ...
    {\
        ...
        {\
            ...
            {\
                ...
                if(x + w >= maxwidth && j!=0) { TEXTLINE(j-1) x = 0; y += FONTH*TEXTSCALE; }\
                TEXTWORD\
            }\
            else\
            { TEXTCHAR(i) }\
