...
bool load_world(const char *mname, const char *cname)        // still supports all map formats that have existed since the earliest cube betas!
{
    ...
    else if(strncmp(hdr.magic, "MAPZ", 4) == 0 || strncmp(hdr.magic, "BFGZ", 4) == 0)
    {
		//mapz &newhdr = (mapz&)hdr;
		mapz newhdr;
		memcpy(&newhdr, &hdr, sizeof(hdr));
        // this check removed below due to breakage: (size_t)hdr.headersize > sizeof(chdr) || f->read(&chdr.worldsize, hdr.headersize-sizeof(binary))!=hdr.headersize-(int)sizeof(binary)
        #define MAPZCOMPAT(ver) \
            ...
            { \
                conoutf("\frerror loading %s: malformatted mapz v%d[%d] header", mname, newhdr.version, ver); \
                delete f; \
                /*maploading = 0;*/ \
                return false; \
            }
        ...
        else
        {
            ...
            {
                conoutf("\frerror loading %s: malformatted mapz v%d header", mname, newhdr.version);
                delete f;
                //maploading = 0;
                return false;
            }
        ...
        if(newhdr.version > MAPZVERSION)
        {
            conoutf("\frerror loading %s: requires a newer version of Red Eclipse", mname);
            delete f;
            //maploading = 0;
            return false;
        }

		copystring(gametype, "mpz");
        resetmap();
        //newhdr = newhdr;
		Texture *mapshot = textureload(picname, 3, true, false);
		renderbackground("loading...", mapshot, mname, game::getmapinfo());
        ...
        //if(newhdr.version <= 24) copystring(newhdr.gameid, "bfa", 4); // all previous maps were bfa-fps
        //if(verbose) conoutf("\faloading v%d map from %s game v%d", newhdr.version, newhdr.gameid, newhdr.gamever);

        if(newhdr.version >= 25 || (newhdr.version == 24 /*&& newhdr.gamever >= 44*/))
        {
            int numvars = newhdr.version >= 25 ? f->getlil<int>() : f->getchar(), vars = 0;
            //identflags |= IDF_WORLD;
            //progress(0, "loading variables...");
            loopi(numvars)
            {
                //if(dbgvars) progress(float(i)/float(numvars), "loading variables...");
                int len = newhdr.version >= 25 ? f->getlil<int>() : f->getchar();
                if(len)
                {
                    string name;
                    f->read(name, len+1);
                    if(newhdr.version <= 34 && !strcmp(name, "cloudcolour")) copystring(name, "cloudlayercolour");
                    ident *id = getident(name);
                    bool proceed = true;
                    int type = newhdr.version >= 28 ? f->getlil<int>()+(newhdr.version >= 29 ? 0 : 1) : (id ? id->type : ID_VAR);
                    if(!id || type != id->type)
                    {
                        if(id && newhdr.version <= 28 && id->type == ID_FVAR && type == ID_VAR)
                            type = ID_FVAR;
                        else proceed = false;
                    }
                    if(!id || !(id->flags&(1<<3))) proceed = false;

                    switch(type)
                    {
                        case ID_VAR:
                        {
                            int val = newhdr.version >= 25 ? f->getlil<int>() : f->getchar();
                            if(proceed)
                            {
                        ...
                        case ID_FVAR:
                        {
                            float val = newhdr.version >= 29 ? f->getlil<float>() : float(f->getlil<int>())/100.f;
                            if(proceed)
                            {
                        ...
                        default:
                        {
                            if(newhdr.version <= 27)
                            {
                                if(newhdr.version >= 25) f->getlil<int>();
                                else f->getchar();
                            }
                            proceed = false;
                            break;
                    ...
                    if(!proceed)
                    {
                        if(dbgvars) conoutf("\frWARNING: ignoring variable %s stored in map", name);
                    }
                    else vars++;
                }
            }
            //identflags &= ~IDF_WORLD;
            if(dbgvars) conoutf("\faloaded %d variables", vars);
        }
