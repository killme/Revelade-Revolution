...
float curfov = 100, curavatarfov = 65, fovy, aspect, scopev = 0;
int farplane;
...
void computezoom()
{
    if(!zoom) { curfov = fov; curavatarfov = avatarfov; scopev = 0; return; }
    if(zoom < 0 && curfov >= fov) { zoom = 0; curfov = fov; curavatarfov = avatarfov; scopev = 0; return; } // don't zoom-out if not zoomed-in
	...
	scopev = (curfov - float(oldfov)) / float(newfov - oldfov);
...
FVAR(thirdpersonheight, 0, 0.4, 1000);
...
void drawscope(int w, int h)
{
    ...
    float fade = scopev>.9f? 1.f: scopev;
	fade = fade<0.1? 1: fade;
    glColor4f(fade, fade, fade, fade);
...
void loadcrosshair(const char *name, int i)
{
    ...
    if(crosshairs[i] == notexture) 
    {
        ...
        if(!name) name = "data/gui/crosshair.png";
...
void drawcrosshair(int w, int h)
{
    ...
    if(!windowhit && (hidehud || mainmenu || cameracap)) return; //(hidehud || player->state==CS_SPECTATOR || player->state==CS_DEAD)) return;
    ...
    else
    {
        ...
        chsize = crosshairsize*(index>3?3:1)*w/900.0f;
    }
