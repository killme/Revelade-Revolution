...
struct serverinfo
{
    ...
    static int compare(serverinfo **ap, serverinfo **bp)
    {
		...
		if (sortserversby == 0 || sortserversby == 1)
		{
			if(a->numplayers < b->numplayers) return 1;
			if(a->numplayers > b->numplayers) return -1;
		}
		if (sortserversby == 0 || sortserversby == 2)
		{
			if(a->ping > b->ping) return 1;
			if(a->ping < b->ping) return -1;
		}
		//if (sortserversby == 0 || sortserversby == 3)
		//{
		//	int cmp = strcmp(a->name, b->name);
		//	if(cmp != 0) return cmp;
		//}
		if (sortserversby == 0 || sortserversby == 4)
		{
			if(a->port < b->port) return -1;
			if(a->port > b->port) return 1;
		}
		if (sortserversby == 0 || sortserversby == 5)
		{
			int cmp = strcmp(a->sdesc, b->sdesc);
			if(cmp != 0) return cmp;
		}
		if ((sortserversby == 0 || sortserversby == 6) && !a->attr.empty() && !b->attr.empty())
		{
			if(a->attr[1] < b->attr[1]) return -1;
			if(a->attr[1] > b->attr[1]) return 1;
		}
		if ((sortserversby == 0 || sortserversby == 7) && a->map[0] && b->map[0])
		{
...
//VARP(filterlocked, 0, 0, 1);
//VARP(filterprivate, 0, 0, 1);
//VARP(filterpassword, 0, 0, 1);
    ...
    for(int start = 0; start < servers.length();)
    {
        ...
        loopi(10)
        {
            ...
            for(int j = start; j < end; j++)
            {
				...
				if (filterunknown && (si.ping < 0 || si.attr.empty() // || si.attr[0] != PROTOCOL_VERSION
					|| si.address.host == ENET_HOST_ANY || si.ping == INT_MAX)) continue;
				if (filterdesc[0])
				{
