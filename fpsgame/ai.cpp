...
namespace ai
{
	...
	{ // add margins of error
        ...
        {
            if(d->gunselect == WEAP_FIST) return true;
        ...
        else // fixed rate logic done out-of-sequence at 1 frame per second for each ai
        {
			...
			loopv(players)
			{
				if(players[i]->ai)
				{
					...
					//if (players[i]->ammo[WEAP_ROCKETL]) players[i]->gunselect = WEAP_ROCKETL;
				}
			}
    ...
    {
		bool hasenemy = enemy(d, b, pos, wander, d->gunselect == WEAP_FIST);
		if(!walk)
		{
    ...
    void damaged(fpsent *d, fpsent *e)
    {
        ...
        {
            aistate &b = d->ai->getstate();
            if(violence(d, b, e, d->gunselect == WEAP_FIST)) return;
        }
        if(checkothers(targets, d, AI_S_DEFEND, AI_T_PLAYER, d->clientnum, true))
        {
            loopv(targets)
            {
                ...
                if(violence(t, c, e, d->gunselect == WEAP_FIST)) return;
            }
        }
    }
    ...
    void setup(fpsent *d, bool tryreset = false)
    {
        ...
        else
        {
			...
			else d->ai->weappref = rnd(WEAP_GRENADIER-WEAP_SLUGSHOT+1)+WEAP_SLUGSHOT;
        }
        vec dp = d->headpos();
        ...
        switch(b.targtype)
        {
            ...
            case AI_T_ENTITY:
                if(entities::ents.inrange(b.target))
                {
                    ...
                    if(!e.spawned || e.type < I_AMMO || e.type > I_AMMO4 || d->hasmaxammo()) return 0;
            ...
            switch(b.targtype)
            {
                ...
                case AI_T_PLAYER:
                {
                    ...
                    if(e && e->state == CS_ALIVE)
                    {
                    	...
                    	if(d->gunselect == WEAP_FIST) guard = 0.f;
    ...
    {
        if(d->gunselect == WEAP_FIST && !d->blocked && !d->timeinair)
        {
    ...
    int process(fpsent *d, aistate &b)
    {
        ...
        if(!enemyok || d->skill >= 50)
        {
            ...
            if(f)
            {
                if(targetable(d, f))
                {
                    if(!enemyok) violence(d, b, f, d->gunselect == WEAP_FIST);
                    enemyok = true;
                    e = f;
                }
                else enemyok = false;
            }
            else if(!enemyok && target(d, b, d->gunselect == WEAP_FIST, false, SIGHTMIN))
    ...
    bool request(fpsent *d, aistate &b)
    {
        ...
        if(!d->hasammo(d->gunselect) || !hasrange(d, e, d->gunselect) || (d->gunselect != d->ai->weappref && (!isgoodammo(d->gunselect) || d->hasammo(d->ai->weappref))))
        {
            ...
            else
            {
				...
				if (!d->hasammo(gun)) gun = WEAP_FIST;
            }
    ...
    void logic(fpsent *d, aistate &b, bool run)
    {
        ...
        if(d->state == CS_ALIVE)
        {
            if(allowmove)
            {
                if(!request(d, b)) target(d, b, d->gunselect == WEAP_FIST, b.idle ? true : false);
                shoot(d, d->ai->target);
            }
            if(!intermission)
            {
    ...
    void render()
    {
        if(aidebug > 1)
        {
            ...
            loopv(players) if(players[i]->state == CS_ALIVE && players[i]->ai)
            {
                ...
                if(aidebug > 2)
                {
					defformatstring(asdf)("%d", d->health);
                    if(d->ai->weappref >= 0 && d->ai->weappref < NUMWEAPS)
                        particle_textcopy(vec(d->abovehead()).add(vec(0, 0, above += 2)), asdf, PART_TEXT, 1);
                    fpsent *e = getclient(d->ai->enemy);
